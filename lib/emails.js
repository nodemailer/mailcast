...
module.exports.emailValidation = async userData => {
    ...
    let info = await mailer.send({
        ...
        message: {
            ...
            from: {
                name: settings.global_site_appName,
                address: 'no-reply@' + settings.global_site_hostname
            },
            to: userData.email
        },
        locals
...
module.exports.emailSubscriptionsToken = async data => {
    let settings = await settingsModel.get('global_site_*');
    ...
    let url = new URL('subscribers/subscriptions/list', locals.appurl);
    url.searchParams.append('t', data.token);
    locals.subscriptionsUrl = url.href;

    let template = 'subscriptions-token';
    let mail = await mailModel.create({
        type: 'transactional',
        template,
        to: data.email

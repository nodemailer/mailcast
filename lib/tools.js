...
module.exports.asyncInterval = async (fn, delay) =>
    new Promise((resolve, reject) => {
        setInterval(() => {
            try {
                fn();
...
module.exports.recaptchaVerify = (req, res, next) => {
    if (!req.recaptchaHandler) {
        req.recaptcha = {
            error: false
        };
        return next();
    ...
    req.recaptchaHandler.middleware.verify(req, res, err => {
        if (err) {
            return next(err);
        }
        if (req.recaptcha.error) {
            ...
            switch (req.recaptcha.error) {
                case 'missing-input-secret':
                    err = new Error('Captcha Error: The secret parameter is missing.');
                    err.status = 500;
                    break;
                case 'invalid-input-secret':
                    err = new Error('Captcha Error: The secret parameter is invalid or malformed.');
                    err.status = 500;
                    break;
                case 'missing-input-response':
                    err = new Error('Captcha Error: The response parameter is missing.');
                    err.status = 500;
                    break;
                case 'invalid-input-response':
                    err = new Error('Captcha Error: The response parameter is invalid or malformed.');
                    err.status = 403;
                    break;
                case 'timeout-or-duplicate':
                    err = new Error('Captcha Error: Token timed out or duplicate request.');
                    err.status = 403;
                    break;
                default:
                    err = new Error('Captcha check failed: ' + req.recaptcha.error);
            }
            return next(err);
        }
        next();
